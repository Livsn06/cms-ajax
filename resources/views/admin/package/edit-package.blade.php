...
<x-layout.admin-app-layout title="Edit Package" :breadcrumbs="$navLinks">

    {{-- Initialize with @js to prevent syntax errors from quotes/special chars --}}
    <div class="p-6" x-data="packageForm(@js($package))">
        ...
        <div x-show="showSuccess" x-cloak x-transition:enter="transition ease-out duration-300"
            ...
            <div class="ms-3 text-sm font-bold">Package updated successfully!</div>
        ...
        <div class="flex items-center justify-between mb-8">
            <div class="space-y-1">
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Edit Package</h1>
                <p class="text-base font-normal text-gray-500">Update the details for <span
                        class="text-orange-600 font-semibold">{{ $package->name }}</span>.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <form @submit.prevent="submitForm" class="bg-white border border-gray-200 rounded-xl shadow-sm">
                        ...
                        <div class="grid grid-cols-6 gap-6">
                            <div class="col-span-6">
                                ...
                                <input type="text" x-model="formData.name" id="name"
                                    ...
                                    :class="errors.name ? 'border-red-500 bg-red-50' : 'border-gray-300'" required>
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" step="0.01" x-model="formData.price" id="price"
                                        ...
                                        :class="errors.price ? 'border-red-500 bg-red-50' : 'border-gray-300'" required>
                                </div>
                    ...
                    <div
                        ...
                        <button type="submit" :disabled="loading"
                            ...
                            <span x-text="loading ? 'Saving...' : 'Update Package'"></span>
                        </button>
                    </div>
                </form>
            </div>
            ...
            <div class="xl:col-span-1">
                <div class="p-6 bg-white border border-gray-200 rounded-xl shadow-sm space-y-4">
                    ...
                    <div>
                        <label class="block mb-2 text-sm font-bold text-gray-700" for="image_path_input">Upload New
                            Cover</label>
                        <input type="file" @change="handleImageChange" id="image_path_input"
    ...
    <script>
        function packageForm(initialData) {
            return {
                ...
                // Use asset helper to generate clean storage URLs
                imageUrl: initialData.image_path ? `{{ asset('storage') }}/${initialData.image_path}` : null,
                errors: {},
                loading: false,
                showSuccess: false,
                ...
                async submitForm() {
                    ...
                    data.append('_method', 'PUT'); // Required for multipart logic in Laravel PUT routes
                    data.append('name', this.formData.name);
                    data.append('price', this.formData.price);
                    data.append('description', this.formData.description);

                    const fileInput = document.getElementById('image_path_input');
                    if (fileInput.files[0]) {
                        data.append('image_path', fileInput.files[0]);
                    }

                    try {
                        const url = "{{ route('admin.packages.update', ':id') }}".replace(':id', this.formData.id);
