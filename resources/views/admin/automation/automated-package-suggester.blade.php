...
<x-layout.admin-app-layout title="Create Package" :breadcrumbs="$navLinks">
    ...
    <div class="p-6" x-data="packageForm()">
        ...
        <div x-show="showSuccess" x-cloak x-transition:enter="transition ease-out duration-300"
            ...
            <div class="ms-3 text-sm font-bold">Package created successfully!</div>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            <div class="space-y-1">
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Package Plan Suggester</h1>
                <p class="text-base font-normal text-gray-500">Configure the pricing and features for your new
                    subscription tier.</p>
            </div>

            <button @click="generateSuggestion" type="button" :disabled="isGenerating"
                class="inline-flex items-center px-5 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-purple-600 via-pink-500 to-orange-400 hover:from-purple-700 hover:to-orange-500 rounded-xl shadow-lg shadow-orange-500/20 transition-all transform hover:scale-105 active:scale-95 disabled:opacity-70">
                ...
                <span x-text="isGenerating ? 'Analyzing Data...' : 'Suggest A Package'"></span>
            </button>
        </div>
                ...
                <form @submit.prevent="submitForm" class="bg-white border border-gray-200 rounded-xl shadow-sm">
                    ...
                    <div
                        ...
                        <button type="button" @click="resetForm"
                            class="text-gray-500 hover:text-gray-700 text-sm font-bold transition-colors">Reset
                            Form</button>
    ...
    <script>
        function packageForm() {
            return {
                ...
                errors: {},
                loading: false,
                isGenerating: false, // AI Loading state
                showSuccess: false,
                ...
                async generateSuggestion() {
                    ...
                    try {
                        const response = await axios.get("{{ route('admin.automation.suggest') }}");
                        const suggestion = response.data;

                        this.formData.name = suggestion.name;
                        this.formData.price = suggestion.price;
